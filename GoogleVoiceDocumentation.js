...
// FIRST THINGS FIRST
/*
First install node-google-voice in the usual manner for node: npm install googlevoice
...
Events are scheduled with voiceClient.scheduler(type,date,...,eventCallback,scheduleCallback) where
	...
	- date (Array or Date) is the time of the event. Events can be scheduled using an array of the form [YEAR,MONTH,DAY,HOUR,MINUTE] (24-hour format) or with a Date object.
...
/*
To remove an event from the schedule, call voiceClient.unscheduler(date) where date is one of the following types:
...
All data requests are of the following form: voiceClient.get(request,limit,callback) where:
	- request (String or Object) is one of the requests.
		'history'
		'inbox'
		'spam'
		'trash'
		...
		'recorded'
			OR
		{query: searchString} - this form retrieves messages that match the given searchString (String) in some form. The search function is entirely implemented by Google Voice, so the search results are the same as would be returned by searching from in the Google Voice web interface.
	...
	- callback (Function) is of the form function(error,messages) where messages is an array of message objects. Each message object is formed from the JSON response from Google Voice; the format is therefore subject to change. At the time of this writing, an example message looked like this:
		...
		NOTE: SMS messages are grouped under one message ID by Google Voice. In order to present all text messages in an SMS thread, an extra processing step occurs for SMS messages which attaches two properties to the message object:
			...
			A convenience method is provided to extract this information from text message DOM elements:
				voiceClient.parseSMS(param,msgDomElement) returns the requested parameter of the text message
