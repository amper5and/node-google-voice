...
* **options** (Object)
	* `options` is required when `method` is `'call'`. The following parameters are recognized:
		...
		* **overwrite** (True/False, default = `true`) : This parameter is recognized only for scheduling requests (i.e. when there is a `date` property in `options`). The `overwrite` property specifies whether a scheduling request can unschedule an event that is scheduled at the same time.
	* `options` is required when `method` is `'sms'`. The following parameters are recognized:
	...
	* `options` is optional for `method == 'cancel'`. If you include it, the following parameters are recognized:
		* **date** (Array or Date, optional) : see above for explanation
		* **overwrite** (True/False, default = `true`) : see above for explanation
